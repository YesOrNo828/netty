...
public class FastJsonProvider implements MessageBodyReader<Object>, MessageBodyWriter<Object> {
    ...
    public static void handleResovleTask(DefaultJSONParser parser, Object value) {
        ...
        for (ResolveTask task : resolveTaskList) {
    ...
    public boolean isWriteable(Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
        return hasMatchingMediaType(mediaType) && isValidType(type, annotations);
    ...
    public void writeTo(Object t, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders,
        ...
        String chaset = null;
        if (null != mediaType && null != mediaType.getParameters()) {
            chaset = mediaType.getParameters().get("charset");
        }
        if (null == chaset || chaset.trim().length() < 1) {
            chaset = "utf-8";
        }
        if (jsonStr != null)
            entityStream.write(jsonStr.getBytes(chaset));
    }
